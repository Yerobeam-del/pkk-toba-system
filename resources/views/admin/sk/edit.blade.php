...
@section('page-title', 'Edit SK & Dokumen')
...
<div class="card">
    <form action="{{ route('admin.sk.update', $dokumen) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input type="text" name="name" class="form-control" value="{{ old('name', $dokumen->name) }}" required>
        </div>
        ...
        <div class="form-group">
            ...
            <div style="margin-top:0.75rem;padding:0.75rem 1rem;background:#f8fafc;border-radius:8px;border:1px solid #e2e8f0">
                <div style="display:flex;align-items:center;gap:0.75rem;margin-bottom:0.5rem">
                    ...
                    <span style="font-weight:500">{{ $dokumen->file_name }}</span>
                    <span style="margin-left:auto;font-size:0.85rem;color:var(--text-muted)">{{ $dokumen->file_size }}</span>
                </div>
                <a href="{{ $dokumen->file_url }}" target="_blank" style="color:var(--primary);font-size:0.85rem;text-decoration:none">🔗 Lihat file saat ini</a>
            </div>
        ...
        <div class="form-group">
            ...
            <div style="display:flex;gap:1.5rem;margin-top:0.5rem">
                <label style="display:flex;align-items:center;gap:8px;cursor:pointer">
                    <input type="radio" name="status" value="published" {{ old('status', $dokumen->status)==='published'?'checked':'' }} style="width:18px;height:18px">
                    <span style="font-weight:500">🟢 Published</span>
                </label>
                <label style="display:flex;align-items:center;gap:8px;cursor:pointer">
                    <input type="radio" name="status" value="draft" {{ old('status', $dokumen->status)==='draft'?'checked':'' }} style="width:18px;height:18px">
        ...
        <div class="modal-footer" style="padding:0;margin-top:2rem">
            <a href="{{ route('admin.sk.index') }}" class="btn btn-outline">Batal</a>
