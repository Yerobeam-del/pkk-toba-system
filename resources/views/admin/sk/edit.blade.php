...
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1.5rem">
    <div>
        <h1 style="font-size:1.5rem;font-weight:800;color:var(--text-dark);margin:0 0 0.25rem 0">Edit Dokumen</h1>
        <p style="color:var(--text-muted);margin:0;font-size:0.9rem">Perbarui data SK atau dokumen yang sudah ada</p>
    </div>
    <a href="{{ route('admin.sk.index') }}" class="btn" style="background:#f8fafc;color:var(--text-dark)">← Kembali</a>
</div>

{{-- Form Card --}}
<div class="card">
    <form action="{{ route('admin.sk.update', $dokumen) }}" method="POST" enctype="multipart/form-data">
        @csrf @method('PUT')
        
        {{-- Document Name --}}
        <div style="margin-bottom:1.5rem">
            <label style="font-weight:600;display:block;margin-bottom:0.5rem;font-size:0.9rem">Nama Dokumen *</label>
            <input type="text" name="name" class="form-control" value="{{ old('name', $dokumen->name) }}" required placeholder="Contoh: SK Pembentukan Panitia PKK 2024">
            <small style="color:var(--text-muted);display:block;margin-top:0.4rem;font-size:0.8rem">Judul dokumen yang akan ditampilkan</small>
        </div>

        {{-- File Upload --}}
        <div style="margin-bottom:1.5rem">
            <label style="font-weight:600;display:block;margin-bottom:0.5rem;font-size:0.9rem">File Dokumen</label>
            ...
            <div id="currentFileInfo" style="margin-top:1rem;padding:0.75rem 1rem;background:#f8fafc;border-radius:10px;display:flex;align-items:center;gap:0.75rem">
                ...
                <div style="flex:1;min-width:0">
                    <div style="font-weight:600;color:var(--text-dark);font-size:0.9rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">{{ $dokumen->file_name }}</div>
                    <div style="font-size:0.8rem;color:var(--text-muted)">{{ $dokumen->file_size }}</div>
                </div>
                <a href="{{ $dokumen->file_url }}" target="_blank" style="color:var(--primary);font-size:0.85rem;text-decoration:none;display:inline-flex;align-items:center;gap:0.25rem">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem;margin-bottom:1.5rem">
            <div>
                <label style="font-weight:600;display:block;margin-bottom:0.5rem;font-size:0.9rem">Tanggal Dokumen *</label>
                <input type="date" name="document_date" class="form-control" value="{{ old('document_date', $dokumen->document_date?->format('Y-m-d')) }}" required>
                <small style="color:var(--text-muted);display:block;margin-top:0.4rem;font-size:0.8rem">Tanggal resmi dokumen</small>
            </div>
            <div>
                <label style="font-weight:600;display:block;margin-bottom:0.5rem;font-size:0.9rem">Urutan Tampil</label>
                <input type="number" name="sort_order" class="form-control" value="{{ old('sort_order', $dokumen->sort_order) }}" min="0">
        ...
        <div style="margin-bottom:2rem">
            ...
            <div style="display:flex;gap:1rem;flex-wrap:wrap">
                <label style="display:flex;align-items:center;gap:0.5rem;padding:0.65rem 1rem;background:#f8fafc;border:1px solid rgba(0,0,0,0.06);border-radius:8px;cursor:pointer;transition:all 0.2s" onmouseover="this.style.borderColor='rgba(34,197,94,0.5)'" onmouseout="this.style.borderColor='rgba(0,0,0,0.06)'">
                    <input type="radio" name="status" value="published" {{ old('status', $dokumen->status)==='published'?'checked':'' }} style="width:18px;height:18px;cursor:pointer">
                ...
                <label style="display:flex;align-items:center;gap:0.5rem;padding:0.65rem 1rem;background:#f8fafc;border:1px solid rgba(0,0,0,0.06);border-radius:8px;cursor:pointer;transition:all 0.2s" onmouseover="this.style.borderColor='rgba(234,179,8,0.5)'" onmouseout="this.style.borderColor='rgba(0,0,0,0.06)'">
                    <input type="radio" name="status" value="draft" {{ old('status', $dokumen->status)==='draft'?'checked':'' }} style="width:18px;height:18px;cursor:pointer">
                    <span style="display:inline-flex;align-items:center;gap:0.4rem;font-weight:500;font-size:0.9rem">
        ...
        <div style="display:flex;gap:0.75rem;justify-content:flex-end;padding-top:1rem;border-top:1px solid rgba(0,0,0,0.04)">
            <a href="{{ route('admin.sk.index') }}" class="btn" style="background:#f8fafc;color:var(--text-dark)">Batal</a>
